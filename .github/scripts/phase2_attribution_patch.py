...
if replacement not in text:
    ...
    if count != 1:
        raise SystemExit(f'attribution helper anchor: expected 1 match, found {count}')
    text = text.replace(anchor, replacement, 1)

endpoint_anchor = '''@router.get("/{slug}/performance")
...
endpoint_replacement = endpoint_anchor + '''
...
if endpoint_replacement not in text:
    count = text.count(endpoint_anchor)
    if count != 1:
        raise SystemExit(f'attribution endpoint anchor: expected 1 match, found {count}')
    text = text.replace(endpoint_anchor, endpoint_replacement, 1)
